...
bool Hooking::HookModuleImport(PLDR_DATA_TABLE_ENTRY moduleHandle, const char* pImportedModuleName, DWORD dwOrdinal, void* pHookFunction) {
	...
	DWORD ordinalAddress = (DWORD)Native::ResolveFunction(pImportedModuleName, dwOrdinal);

	if (!ordinalAddress) {
		LOG_PRINT("Failed to resolve ordinal 0x%X from %s", dwOrdinal, pImportedModuleName);
		return false;
	}
	...
	if (!imports) {
		LOG_PRINT("Failed to find import descriptors");
		return false;
	}
...
bool Hooking::HookModuleImport(const char* pModuleName, const char* pImportedModuleName, DWORD dwOrdinal, void* pHookFunction) {
	LDR_DATA_TABLE_ENTRY* moduleHandle = (LDR_DATA_TABLE_ENTRY*)GetModuleHandle(pModuleName);

	if (!moduleHandle) {
		LOG_PRINT("Failed to find %s", pModuleName);
		return false;
	}
...
void Hooking::HookFunctionStart(DWORD* dwAddress, DWORD* dwSaveStub, DWORD dwDestination) {
	if ((dwSaveStub != NULL) && (dwAddress != NULL)) {
		...
		LOG_PRINT("A function failed to hook, :(");
	}
}
