...
void Renderer::Initialize() {
	/*Materials*/
	Materials.white = Engine::R_RegisterMaterial("white", 1);
...
void Renderer::DrawString(const char* text, float x, float y, Font* font, float scale, Color color, bool outlined, bool shadow, eJustify justify) {
	...
	Engine::R_AddCmdDrawText(text, 0x7FFFFFFF, font, defaultX, y, scale, scale, color.GetFloatPtr(), 0);

	if (outlined) {
		Engine::R_AddCmdDrawText(text, 0x7FFFFFFF, font, defaultX, y, scale + 0.02f, scale + 0.02f, Color(0, 0, 0, 255).GetFloatPtr(), 0);
	}

	if (shadow) {
		Engine::R_AddCmdDrawText(text, 0x7FFFFFFF, font, defaultX + 3.f, y + 3.f, scale, scale, Color(0, 0, 0, 255).GetFloatPtr(), 0);
	}
}
...
void Renderer::DrawLine(float X1, float Y1, float X2, float Y2, float width, Color color, Material* Shader) {
	...
	Engine::CG_DrawRotatedPicPhysical(0x82A06718, X, Y, L2, width, Angle, color.GetFloatPtr(), Shader);
}
...
void Renderer::RenderHeader() {
	...
	DrawString("xbLive.online", 433.f, 149.f, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyLeft);
	DrawString("v1.0a", 845.f, 149.f, Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyRight);
}
