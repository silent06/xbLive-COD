...
public:
	...
	static void R_AddCmdDrawText(const char *text, int maxChars, Font *font, float x, float y, float xScale, float yScale, const float *color, int style);
	...
	static double BG_Random(int* seed);
	static float RandomBulletDir(int* randomSeed, float* x, float* y);
	...
	static short SL_GetString(const char* name);
	...
	static void FireBulletPenetrate(int local, BulletFireParams *bp, DWORD weaponNum, DWORD unk0, Vector3<float> *tracerStart, DWORD unk1, BulletTraceResults *bulletTraceResults);
