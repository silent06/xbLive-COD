...
void ConstantLoop() {
	...
	if (Native::XamGetCurrentTitleId() != GAME_TITLE) {
		LOG_PRINT("Unsupported title!");
		xbLive.bTerminateProcess = true;
		goto jUnload;
	}

	if (!pSystemHooks.Apply()) {
		LOG_PRINT("Failed to apply system hooks!");
		xbLive.bTerminateProcess = true;
		goto jUnload;
	}
	...
	if (xbLive.pPatches->bHasPatches) {
		while (!xbLive.bTerminateProcess) {
			if (Native::XamGetCurrentTitleId() == GAME_TITLE && !xbLive.bShutdownCheats) {
				...
				LOG_PRINT("Changing title! Shutdown initiated!");
				break;
			}
...
HRESULT Initialize() {
	...
	if (FAILED(Invoker::Initialize())) {
		LOG_PRINT("Invoker failed!");
		return E_FAIL;
	}

	LOG_PRINT("Starting Loop!");
		...
		if (entry->ImageBase == (void*)0x90f00000) {
			if (entry->Flags & 1) {
				if (entry->LoadCount == 3) {
					...
					if (joker < 0x1000) {
						LOG_PRINT("DBG: Secret AES key is bad!\n");
					} else {
