...
void NumberOption<Type>::Render(int pos) {
	...
	case TOGGLE: {
		...
		pRenderer.DrawString(text, 800 - 13, 195 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.bodyFont, 0.73f, Color(0xFFFFFFFF), false, false, JustifyCenter);
		break;
	}
	...
	case SCROLLSELECT:
		...
		auto text = pUtils.va(pFormat, *pNumber);
		pRenderer.DrawString(text, 800 - 8, 195 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.bodyFont, 0.73f, Color(0xFFFFFFFF), false, false, JustifyCenter);
		break;
	}
