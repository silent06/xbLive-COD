...
public:
	...
	void Render(int pos) {
		...
		case TOGGLE: {
			...
			pRenderer.DrawString(text, 820, 196 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyRight);
			break;
		}
		...
		default:
			auto text = pScrollStruct[*pNumber].pName;
			pRenderer.DrawString(text, 845, 196 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.bodyFont, 0.76f, Color(0xFFFFFFFF), false, false, JustifyRight);
			break;
		}
