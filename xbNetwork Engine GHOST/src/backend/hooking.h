...
public:
	...
	template <typename T>
	bool HookFunction(DWORD dwAddress, void* pHookFunction, T* pTrampoline, bool addToIntegrity = false) {
		if (dwAddress) {
			HookContext cxt;
	...
	/*bool HookFunction(DWORD dwAddress, void* pHookFunction, void** pTrampoline) {
		HookContext cxt;
		cxt.dwAddress = dwAddress;
		memcpy(cxt.szAsm, (void*)dwAddress, 0x10);
		vHookContext.push_back(cxt);
		return Native::HookFunction(dwAddress, pHookFunction, pTrampoline, szHookSection, &iHookCount);
	}*/
};
	...
	virtual DWORD DetourFunctionStart(DWORD dwFunctionAddress, DWORD dwStubAddress, PVOID pDestFunc) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (b == 0x48 || b == 0x4B) {
				...
				pUtils.PatchInJump((PDWORD)(dwStubAddress + dwLength), dwTemp, bTemp);
				dwLength += 0x10;
			...
			// beq or bne, ble or bgt, bge or blt
				...
				branch_else:
					...
					*(DWORD *)(dwStubAddress + dwLength) = ((0x40000000 + (*(DWORD *)dwTempFuncAddr & 0x00FF0000) + 0x14) +
						bTemp ? 0 : 0x01000000);
					dwLength += 4;
					pUtils.PatchInJump((PDWORD)(dwStubAddress + dwLength), (dwTempFuncAddr + dwTemp), FALSE);
					dwLength += 0x10;
				}
			}
		...
		pUtils.PatchInJump((PDWORD)(dwStubAddress + dwLength), (dwFunctionAddress + 0x10), FALSE);
		dwLength += 0x10;

	DoHook:
		// apply the hook
		pUtils.PatchInJump((PDWORD)dwFunctionAddress, (DWORD)pDestFunc, FALSE);
		return dwLength;
	}
