...
void NumberOption<Type>::Render(int pos) {
	...
	case TOGGLE: {
		...
		pRenderer.DrawString(text, 800 - 13, 196 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.normalFont, 0.47f, Color(0xFFFFFFFF), false, false, JustifyCenter);
		break;
	}
	...
	case SCROLLSELECT:
		...
		auto text = pUtils.va(pFormat, *pNumber);
		pRenderer.DrawString(text, 800 - 8, 196 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.normalFont, 0.47f, Color(0xFFFFFFFF), false, false, JustifyCenter);
		break;
	}
