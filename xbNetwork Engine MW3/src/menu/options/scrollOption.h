...
public:
	...
	void Render(int pos) {
		...
		case TOGGLE: {
			...
			pRenderer.DrawString(text, 820, 195 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyRight);
			break;
		}
		...
		default:
			auto text = pScrollStruct[*pNumber].pName;
			pRenderer.DrawString(text, 845, 195 + (pos * pRenderer.fOptionHeight), pRenderer.Fonts.normalFont, 0.50f, Color(0xFFFFFFFF), false, false, JustifyRight);
			break;
		}
