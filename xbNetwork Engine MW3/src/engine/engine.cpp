...
Material* Engine::R_RegisterMaterial(const char* pFont, int iImageTrack) {
	return Invoke::Call<Material*>(GetPatch(xbLive.pPatches->MW3.R_RegisterMaterial), pFont, iImageTrack);
}

Font* Engine::R_RegisterFont(const char* pFont, int iImageTrack) {
	return Invoke::Call<Font*>(GetPatch(xbLive.pPatches->MW3.R_RegisterFont), pFont, iImageTrack);
}
...
void Engine::R_AddCmdDrawStretchPic(float x, float y, float width, float height, float xScale, float yScale, float xay, float yay, const float * color, Material* material) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.R_AddCmdDrawStretchPic), x, y, width, height, xScale, yScale, xay, yay, color, material);
}
...
void Engine::R_AddCmdDrawText(const char *text, int maxChars, Font *font, float x, float y, float xScale, float yScale, float rotation, const float *color, int style) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.R_AddCmdDrawText), text, maxChars, font, x, y, xScale, yScale, rotation, color, style);
};

int Engine::R_TextWidth(const char* text, Font* font) {
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->MW3.R_TextWidth), text, 0x7FFFFFFF, font);
}
...
void* Engine::CG_DrawRotatedPicPhysical(int scrPlacement, float x, float y, float width, float height, float angle, const float* color, Material* material) {
	return Invoke::Call<void*>(GetPatch(xbLive.pPatches->MW3.CG_DrawRotatedPicPhysical), scrPlacement, x, y, width, height, angle, color, material);
}
...
int Engine::R_NormalizedTextScale(Font* font, float scale) {
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->MW3.R_NormalizedTextScale), font, scale);
}
...
void Engine::CG_CompassDrawPlayerMap(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, Material* material, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawPlayerMap), localClientNum, compassType, parentRect, rect, material, color);
}

void Engine::CG_CompassDrawPlayer(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawPlayer), localClientNum, compassType, parentRect, rect, color);
}
...
void Engine::CG_CompassDrawCrates(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, Material* material, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawCrates), localClientNum, compassType, parentRect, rect, material, color);
}
...
void Engine::CG_CompassDrawPlanes(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, Material* material, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawPlanes), localClientNum, compassType, parentRect, rect, material, color);
}
...
void Engine::CG_CompassDrawFriendlies(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawFriendlies), localClientNum, compassType, parentRect, rect, color);
}

void Engine::CG_CompassDrawEnemies(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawEnemies), localClientNum, compassType, parentRect, rect, color);
}
...
void Engine::CG_CompassDrawMissiles(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawMissiles), localClientNum, compassType, parentRect, rect, color);
}

void Engine::CG_CompassDrawTurrets(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawTurrets), localClientNum, compassType, parentRect, rect, color);
}
...
void Engine::CG_CompassDrawHelicopter(int localClientNum, CompassType compassType, RectDef* parentRect, RectDef* rect, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_CompassDrawHelicopter), localClientNum, compassType, parentRect, rect, color);
}
...
bool Engine::CG_WorldPosToCompass(CompassType compassType, cg* cg, RectDef* mapRect, float* north, float* world, Vector3<float> origin, float* out, float* outClipped) {
	return Invoke::Call<bool>(GetPatch(xbLive.pPatches->MW3.CG_WorldPosToCompass), compassType, cg, mapRect, north, world, origin, out, outClipped);
}

void Engine::CG_Trace(Trace* trace, Vector3<float>* start, Vector3<float>* end, int ignore, int mask) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_Trace), trace, start, end, ignore, mask);
}
...
void Engine::FireBulletPenetrate(int client, BulletFireParams* bullet, int weapon, int attacker, Entity* entity, int alwaysMinusOne, Vector3<float>* viewOrigin, bool unkBool) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.FireBulletPenetrate), client, bullet, weapon, attacker, entity, alwaysMinusOne, viewOrigin, unkBool);
}

void Engine::CL_SetStance(int client, int stance) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CL_SetStance), client, stance);
}

void Engine::CG_GetEntityBModelBounds(Entity* entity, Vector3<float>* min, Vector3<float>* max) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_GetEntityBModelBounds), entity, min, max);
}

int Engine::CG_GetWeaponIndexForName(const char* name) {
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->MW3.CG_GetWeaponIndexForName), name);
}

bool Engine::CG_CalcNamePosition(int client, Vector3<float>* out, float* outx, float* outy) {
	return Invoke::Call<bool>(GetPatch(xbLive.pPatches->MW3.CG_CalcNamePosition), client, out, outx, outy);
}

void Engine::Cbuf_AddText(int localPlayer, const char* command) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.Cbuf_AddText), localPlayer, command);
}

bool Engine::CG_DObjGetWorldTagPos(Entity* entity, int dObj, DWORD tag, Vector3<float>* out) {
	return Invoke::Call<bool>(GetPatch(xbLive.pPatches->MW3.CG_DObjGetWorldTagPos), entity, dObj, tag, out);
}

bool Engine::AimTarget_GetTagPos(int localClientNum, Entity* entity, short tagIndex, Vector3<float>* out) {
	return Invoke::Call<bool>(GetPatch(xbLive.pPatches->MW3.AimTarget_GetTagPos), localClientNum, entity, tagIndex, out);
}
...
int Engine::Com_GetClientDObj(int handle, int something) {
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->MW3.Com_GetClientDObj), handle, something);
}

DWORD Engine::SL_GetString(const char* str) {
	return Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.SL_GetString), str, 0);
}

int Engine::AimAssist_GetTagPos(int mask, Entity* entity, short tag, Vector3<float>* out) {
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->MW3.AimAssist_GetTagPos), mask, entity, tag, out);
}

void Engine::CG_DrawGrenadePointer(int localClientNum, float centerX, float centerY, Vector3<float>* grenadeOffset, float* color) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_DrawGrenadePointer), localClientNum, centerX, centerY, grenadeOffset, color);
}
...
void Engine::CG_DrawGrenadeIcon(int localClientNum, float centerX, float centerY, Vector3<float>* grenadeOffset, float* color, Material* material) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_DrawGrenadeIcon), localClientNum, centerX, centerY, grenadeOffset, color, material);
}

void Engine::RandomBulletDir(int* randomSeed, float* x, float* y) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.RandomBulletDir), randomSeed, x, y);
}

void Engine::BG_SeedRandWithGameTime(int* seed) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.BG_SeedRandWithGameTime), seed);
}

void Engine::BG_GetSpreadForWeapon(cg* ps, int weapon, float* minSpread, float* maxSpread) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.BG_GetSpreadForWeapon), ps, weapon, minSpread, maxSpread);
}

void Engine::CG_GetPlayerViewOrigin(int client, cg* ps, Vector3<float>* out) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.CG_GetPlayerViewOrigin), client, ps, out);
}

void Engine::VectorToAngle(Vector3<float>* direction, Vector3<float>* out) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.VectorToAngle), direction, out);
}

float Engine::BG_Random(int* pSeed) {
	return Invoke::Call<float>(GetPatch(xbLive.pPatches->MW3.BG_Random), pSeed);
}

Entity* Engine::GetEntityPtr() {
	return *(Entity**)GetPatch(xbLive.pPatches->MW3.GetEntityPtr);
}
...
cg* Engine::GetCG() {
	return *(cg**)GetPatch(xbLive.pPatches->MW3.GetCG);
}
...
ServerInfo* Engine::GetServerInfo() {
	return *(ServerInfo**)GetPatch(xbLive.pPatches->MW3.GetServerInfo);
}

ClientActive* Engine::GetClientActive() {
	return *(ClientActive**)GetPatch(xbLive.pPatches->MW3.GetClientActive);
}

_XenonUserData Engine::GetXenonUserData(int index) {
	_XenonUserData* XenonUserData = (_XenonUserData*)GetPatch(xbLive.pPatches->MW3.GetXenonUserData);
	if (XenonUserData) {
...
bool Engine::InGame() {
	return GetDvar<bool>(GetPatch(xbLive.pPatches->MW3.cl_ingame)) && GetCG() != nullptr;
}

DWORD Engine::TimeGetTime() {
	return Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->MW3.TimeGetTime));
}
...
bool Engine::IsFriend(int client) {
	...
	return cg->ClientInfo[client].iTeam == cg->ClientInfo[cg->iClientNumber].iTeam && strcmp(GetDvar<const char*>(GetPatch(xbLive.pPatches->MW3.ui_gametype)), "dm");
}
...
WeaponDef* Engine::GetWeaponDef(int weapon) {
	int weapDefId = ((int(*)(...))GetPatch(xbLive.pPatches->MW3.GetWeaponDefID))(weapon, 0);
	return *(WeaponDef**)(GetPatch(xbLive.pPatches->MW3.GetWeaponDef) + ((weapDefId * 4) & 0x3FC));
}
