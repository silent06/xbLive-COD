...
Material* Engine::R_RegisterMaterial(const char* pFont, int iImageTrack) {
	return Invoke::Call<Material*>(GetPatch(xbLive.pPatches->BO2.R_RegisterMaterial), material, pFont, iImageTrack);
}

Font* Engine::R_RegisterFont(const char* pFont, int iImageTrack) {
	return Invoke::Call<Font*>(GetPatch(xbLive.pPatches->BO2.R_RegisterMaterial), font, pFont, iImageTrack);
}
...
void Engine::R_AddCmdDrawStretchPic(float x, float y, float width, float height, float xScale, float yScale, float xay, float yay, const float * color, Material* material) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.R_AddCmdDrawStretchPic), x, y, width, height, xScale, yScale, xay, yay, color, material);
}
...
void Engine::R_AddCmdDrawText(const char *text, int maxChars, Font *font, float x, float y, float xScale, float yScale, float rotation, const float *color, int style) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.R_AddCmdDrawText), text, maxChars, font, x, y, xScale, yScale, rotation, color, style);
}

int Engine::R_TextWidth(const char* text, Font* font) {
	if (font == nullptr) return 0;
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->BO2.R_TextWidth), 1, text, 0x7FFFFFFF, font);
}
...
void Engine::CG_DrawRotatedPicPhysical(int scrPlacement, float x, float y, float width, float height, float angle, const float* color, Material* material) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.CG_DrawRotatedPicPhysical), scrPlacement, x, y, width, height, angle, color, material);
}
...
int Engine::R_NormalizedTextScale(Font* font, float scale) {
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->BO2.R_NormalizedTextScale), font, scale);
}

bool Engine::InGame() {
	return GetDvar<bool>(GetPatch(xbLive.pPatches->BO2.cl_ingame)) && GetCG() != nullptr;
}

cg* Engine::GetCG() {
	return *(cg**)GetPatch(xbLive.pPatches->BO2.GetCG);
}

ServerInfo* Engine::GetServerInfo() {
	return *(ServerInfo**)GetPatch(xbLive.pPatches->BO2.GetServerInfo);
}

DWORD Engine::GetNetInfoAddress(int index) {
	return (GetPatch(xbLive.pPatches->BO2.GetNetInfoAddress) + (index * 0x148));
}

DWORD Engine::TimeGetTime() {
	return Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.TimeGetTime));
}
...
void Engine::Cbuf_AddText(int localClient, const char* msg) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.Cbuf_AddText), localClient, msg);
}

void Engine::CL_AddReliableCommand(int localClient, const char* msg) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.CL_AddReliableCommand), localClient, msg);
}

DWORD Engine::TransformSeed(int* pSeed) {
	return Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.TransformSeed), pSeed);
}

void Engine::BG_GetSpreadForWeapon(DWORD ps, int weapon, float* minSpread, float* maxSpread) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.BG_GetSpreadForWeapon), ps, weapon, minSpread, maxSpread);
}

float Engine::RandomBulletDir(int* randomSeed) {
	return Invoke::Call<float>(GetPatch(xbLive.pPatches->BO2.RandomBulletDir), randomSeed);
}

Entity* Engine::GetEntityPtr() {
	return *(Entity**)GetPatch(xbLive.pPatches->BO2.GetEntityPtr);
}
...
bool Engine::IsAlive(int index, bool player) {
	...
	if (player) {
		if (entity.Type != ET_PLAYER && entity.Type != ET_VEHICLE_CORPSE)
			return false;
	}

	if ((*(int*)((DWORD)&entity + 0x36C) & 0x40000000) == 0 || (*(int*)((DWORD)&entity + 0x1D4) & 0x40000))
		return false;
...
int Engine::Com_GetClientDObj(int handle, int something) {
	return Invoke::Call<int>(GetPatch(xbLive.pPatches->BO2.Com_GetClientDObj), handle, something);
}

bool Engine::CG_DObjGetWorldTagPos(Entity* entity, int dObj, DWORD tag, Vector3<float>* out) {
	return Invoke::Call<bool>(GetPatch(xbLive.pPatches->BO2.CG_DObjGetWorldTagPos), entity, dObj, tag, out);
}

short Engine::SL_GetString(const char* name, DWORD zero) {
	return Invoke::Call<short>(GetPatch(xbLive.pPatches->BO2.SL_GetString), name, zero);
}

const char* Engine::SL_ConvertToString(short tag) {
	return Invoke::Call<const char*>(GetPatch(xbLive.pPatches->BO2.SL_ConvertToString), tag);
}
...
bool Engine::CG_GetPlayerViewOrigin(int local, DWORD ps, Vector3<float>* origin) {
	return Invoke::Call<bool>(GetPatch(xbLive.pPatches->BO2.CG_GetPlayerViewOrigin), local, ps, origin);
}
...
float Engine::GetBoneFraction(int client, const char* tag) {
	...
	Vector3<float> pos;
	if (Engine::CG_DObjGetWorldTagPos(&GetEntity(client), (int)dobj, SL_GetString(tag, 0), &pos)) {
		Vector3<float> myPos;
...
void Engine::CG_LocationalTrace(Trace *trace, Vector3<float> *start, Vector3<float> *end, int passEntityNum, int contMask, int u1, int u2) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.CG_LocationalTrace), trace, start, end, passEntityNum, contMask, u1, u2);
}
...
Vector3<float> Engine::GetTagPosition(int client, const char* tag, bool cached) {
	...
	Vector3<float> out;
	if (Engine::CG_DObjGetWorldTagPos(&GetEntity(client), dobj, SL_GetString(tag, 0), &out)) {
		return out;
	}
...
void Engine::FireBulletPenetrate(int local, BulletFireParams *bp, DWORD weaponNum, Entity* centity_s, Vector3<float>* Origin, DWORD one, bool unk, BulletTraceResults *br) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.FireBulletPenetrate), local, bp, weaponNum, centity_s, Origin, one, unk, br);
}

ClientActive* Engine::GetClientActive() {
	return *(ClientActive**)GetPatch(xbLive.pPatches->BO2.GetClientActive);
}

DWORD Engine::CG_GetWeaponIndexForName(const char* name) {
	return Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.CG_GetWeaponIndexForName), name, 0);
}
...
WeaponDef* Engine::GetWeaponDef(int weapon) {
	return Invoke::Call<WeaponDef*>(GetPatch(xbLive.pPatches->BO2.GetWeaponDef), weapon);
}
...
void Engine::CG_DrawGrenadePointer(int client, float centerX, float centerY, Vector3<float>* offset, float* color, float radiusOffset, float scaleFactor) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.CG_DrawGrenadePointer), client, centerX, centerY, offset, color, radiusOffset, scaleFactor);
}
...
void Engine::CG_DrawGrenadeIcon(int client, float centerX, float centerY, Vector3<float>* offset, float* color, Material* material, float radiusOffset, float scaleFactor) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.CG_DrawGrenadeIcon), client, centerX, centerY, offset, color, material, radiusOffset, scaleFactor);
}

const char* Engine::Dvar_GetString(const char* dvar) {
	return Invoke::Call<const char*>(GetPatch(xbLive.pPatches->BO2.Dvar_GetString), dvar);
}

void Engine::CL_SetStance(int local, int stance) {
	Invoke::Call<DWORD>(GetPatch(xbLive.pPatches->BO2.CL_SetStance), local, stance);
}

DObjAnimMat* Engine::CG_DObjGetLocalTagMatrix(Entity* entity, DObj_s* dObj, unsigned int tagName) {
	return Invoke::Call<DObjAnimMat*>(GetPatch(xbLive.pPatches->BO2.CG_DObjGetLocalTagMatrix), entity, dObj, tagName);
}

DObjAnimMat* Engine::CG_DObjGetWorldTagMatrix(Entity* entity, DObj_s* dObj, unsigned int tagName, Vector3<float>* axis, Vector3<float>* origin) {
	return Invoke::Call<DObjAnimMat*>(GetPatch(xbLive.pPatches->BO2.CG_DObjGetWorldTagMatrix), entity, dObj, tagName, axis, origin);
}
...
Material* Engine::GetWeaponMaterialWithAspect(int weapon, Vector2<float>* aspect) {
	...
	if (weaponDef) {
		GetWeaponKillIconAspect(weaponDef->hudIconRatio, aspect);
		return weaponDef->pKillIcon;
	}
